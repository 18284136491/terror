...
class pagination{
    ...
    public  function fetchAll(){
    	if( $this->isArray){
	    	...
	    	$result = $this->start();
	    	while ($array = $this->db->fetchArray($result, MYSQLI_ASSOC)){
					$temp[] = $array;        		
	       }
    	}
       return (array)$temp;
    ...
    public  function gain_total(){
        if( $this->total > 0){ //设置了最大记录数
			...
			$this->total = $this->db->numRows($result);	
		}else{
			$array = $this->db->getOne( $this->tquery, MYSQLI_NUM);
    ...
    private function gain_left( $pg){
        $array = $this->method=='POST' ? (array)$_REQUEST : (array)$_GET;
